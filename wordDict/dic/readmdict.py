    ...
    def _decode_key_block(self, key_block_compressed, key_block_info_list):
        ...
        for compressed_size, decompressed_size in key_block_info_list:
            ...
            if key_block_type == "\x00\x00\x00\x00":
                ...
                assert (
                    key_block_compressed[start + 4 : start + 8]
                    == key_block_compressed[end - 4 : end]
                )
                # decompress key block
    ...
    def _decode_record_block(self):
        ...
        for compressed_size, decompressed_size in record_block_info_list:
            ...
            while i < len(self._key_list):
                ...
                # convert to utf-8
                record = (
                    record.decode(self._encoding, errors="ignore").strip(u"\x00").encode("utf-8")
                )
                # substitute styles
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--substyle", action="store_true", help="substitute style definition if present"
    )
    parser.add_argument(
        "-d", "--datafolder", default="data", help="folder to extract data files from mdd"
    )
    parser.add_argument(
